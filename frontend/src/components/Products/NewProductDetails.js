...
import { addNewItemsToCart } from "../../actions/CartAction";
import { addNewFavouriteItemsToCart } from "../../actions/FavouriteAction";
import { NEW_NEWREVIEW_RESET } from "../../constans/NewProductConstans";
...
const NewProductDetails = ({ match, history }) => {
  const dispatch = useDispatch();
  const { newproduct, loading, error } = useSelector(
  ...
  useEffect(() => {
    ...
    dispatch(getNewProductDetails(match.params.id));
  ...
  const addToCartHandler = () => {
    if (newproduct.Stock > 0) {
      dispatch(addNewItemsToCart(match.params.id, quantity));
  ...
  const addToFavouriteHandler = () => {
    dispatch(addNewFavouriteItemsToCart(match.params.id, quantity));
    toast.success("Product Added to Favourites");
  };

  // slider

  const len = newproduct.images?.length - 1;
  ...
  return (
    <>
      {loading ? (
        ...
        <>
          <MetaData title={`${newproduct.name}`} />
          <Header />
          <div className="ProductDetails">
            ...
            <div className="varse__2">
              ...
              <div className="detailsBlock-2">
                <Rating {...options} />
                <span>({newproduct.numOfReviews} Reviews)</span>
              </div>
              <div className="detailsBlock">
                ...
                >
                  <h1 className="discountPrice">
                    {newproduct.offerPrice > 0
                      ? `Kshs.${newproduct.offerPrice}`
                      : ""}
                  </h1>
                  <h1>{`Kshs.${newproduct.price}`}</h1>
                </div>
                ...
                <p className="stock__meta">
                  <b
                    className={newproduct.Stock < 1 ? "redColor" : "greenColor"}
                  >
                    {newproduct.Stock < 1 ? "Out Of Stock" : "InStock "}
                  </b>
                  <p style={{ marginLeft: "5px" }}>
                    {newproduct.Stock < 1
                      ? "(0 remaining)"
                      : `(${newproduct.Stock} remaining)`}
                  </p>
                </p>
